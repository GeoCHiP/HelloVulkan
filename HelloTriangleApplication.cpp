...
static const int s_MAX_FRAMES_IN_FLIGHT = 2;

static const uint32_t s_WIDTH = 800;
static const uint32_t s_HEIGHT = 600;
...
bool HelloTriangleApplication::isDeviceSuitable(VkPhysicalDevice device) {
    ...
    bool retVal = indices.isComplete();
    retVal = retVal && extensionsSupported;
    retVal = retVal && swapChainAdequate;
    retVal = retVal && deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU;
    retVal = retVal && deviceFeatures.geometryShader;
...
void HelloTriangleApplication::CreateCommandBuffers() {
    m_CommandBuffers.resize(s_MAX_FRAMES_IN_FLIGHT);
...
void HelloTriangleApplication::CreateSyncObjects() {
    m_ImageAvailableSemaphores.resize(s_MAX_FRAMES_IN_FLIGHT);
    m_RenderFinishedSemaphores.resize(s_MAX_FRAMES_IN_FLIGHT);
    m_InFlightFences.resize(s_MAX_FRAMES_IN_FLIGHT);
    ...
    for (size_t i = 0; i < s_MAX_FRAMES_IN_FLIGHT; ++i) {
        auto resSemaphoreImageAvail = vkCreateSemaphore(m_Device, &semaphoreInfo, nullptr, &m_ImageAvailableSemaphores[i]);
        auto resSemaphoreRenderFin = vkCreateSemaphore(m_Device, &semaphoreInfo, nullptr, &m_RenderFinishedSemaphores[i]);
        auto resFence = vkCreateFence(m_Device, &fenceInfo, nullptr, &m_InFlightFences[i]);

        if (resSemaphoreImageAvail != VK_SUCCESS || resSemaphoreRenderFin != VK_SUCCESS || resFence != VK_SUCCESS) {
            throw std::runtime_error("Failed to create syncronization objects!");
        }
    }
}
...
void HelloTriangleApplication::DrawFrame() {
    ...
    m_CurrentFrame = (m_CurrentFrame + 1) % s_MAX_FRAMES_IN_FLIGHT;
}
